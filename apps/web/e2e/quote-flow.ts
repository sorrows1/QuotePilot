...
export async function quoteFlow(admin: Page, sales: Page, info: TestInfo) {
  ...
  await sales.getByLabel('Selected customer', { exact: true }).selectOption({ label: 'Synthetic Industrial Supplies' })
  ...
  await sales.getByLabel('Pricing UOM', { exact: true }).fill('EA')
