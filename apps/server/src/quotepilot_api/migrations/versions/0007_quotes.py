...
def upgrade() -> None:
    op.execute("""
        CREATE TABLE quote_cases (
          ...
          customer_id uuid NOT NULL, creator_id uuid NOT NULL, created_at timestamptz NOT NULL,
        ...
        CREATE TABLE quote_revisions (
          ...
          FOREIGN KEY (tenant_id,case_id,customer_id)
            REFERENCES quote_cases(tenant_id,id,customer_id),
          FOREIGN KEY (tenant_id,creator_id) REFERENCES users(tenant_id,id),
        ...
        CREATE FUNCTION qt008_case_customer_immutable() RETURNS trigger LANGUAGE plpgsql AS $$
        BEGIN
          IF NEW.customer_id IS DISTINCT FROM OLD.customer_id THEN
            RAISE EXCEPTION 'Quote case customer is immutable' USING ERRCODE = '23514';
          END IF;
          RETURN NEW;
        END $$;
        CREATE TRIGGER immutable_case_customer BEFORE UPDATE OF customer_id ON quote_cases
          FOR EACH ROW EXECUTE FUNCTION qt008_case_customer_immutable();
...
def downgrade() -> None:
    op.execute("""
        ...
        DROP FUNCTION qt008_case_customer_immutable();
